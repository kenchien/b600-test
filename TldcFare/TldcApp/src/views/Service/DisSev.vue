  ...
  <div>
    ...
    <ValidationObserver ref="obs">
      <v-row dense class="ml-3">
          ...
          <v-btn class="ml-3 mt-3 warning" @click="ExecSevStatusChange()" :disabled="haveExec">執行失格/復效</v-btn>
          <v-btn class="ml-3 mt-3" @click="downloadExcel()" :disabled="!haveExec" >下載Excel</v-btn>
          <v-col cols="12">
            <v-label class="info">本次計算的繳費期限=本月1日或16日,TTKK=2個月前第二期(GG)</v-label>
          </v-col>
          <v-col cols="12">
            <v-label class="info">例如操作者在9/3執行此功能時, 若服務人員7月的GG還沒繳費的話,就會正常->失格 (正常繳費期限6月底,最晚寬限到8月底)</v-label>
          </v-col>
          <v-col cols="12">
            <v-label class="info">然後服務人員於9/15補繳6月PP(9/16以前),則操作者在9/22執行此功能時,就會把失格->正常</v-label>
          </v-col>
          <v-col cols="12">
            <v-label class="info">7月的GG,9/1未繳就失格(+2月),明年2/1未繳就KK(+7月)</v-label>
          </v-col>
          <v-col cols="12">
...
export default {
  beforeMount() {
    ...
    if(tempDay<16){
      ...
      this.searchItem.issueYm =  moment().add(-2,"month").format("YYYYMM2");
    }else{
  ...
  methods: {
    //1.執行失格/復效
    ExecSevStatusChange() {
      if (!confirm("確定要執行"+this.searchItem.issueYm+"的服務人員失格/復效?")) return;

      this.$refs.obs.validate().then((re) => {
        if (re) {
          this.execMsg = "";
          this.searchItem.temp = "2-8";//特殊,設定條件,只撈取會員改狀態的資料
          this.$store.dispatch("ExecSevStatusChange", { searchItem:this.searchItem })
          .then((res) => {
    ...
    //2.下載Excel
    downloadExcel() {
      this.searchItem.temp = "2-8";//特殊,設定條件,只撈取服務人員改狀態的資料
      this.searchItem.reportId = "2-8";
