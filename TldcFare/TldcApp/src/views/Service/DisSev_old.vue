  ...
  <div>
    ...
    <ValidationObserver ref="obs">
      <v-row dense class="ml-1">
        ...
        <v-col cols="12" sm="2">
          <v-btn class="mt-3" @click="ExecSevStatusChange(changeDate)" color="success">直接執行</v-btn>
        </v-col>
        <v-col cols="12" sm="2">
...
export default {
  ...
  methods: {
    //執行服務人員失格/KK
    ExecSevStatusChange(changeDate) {
      this.$refs.obs.validate().then((re) => {
        if (re) {
          this.haveExec = true;//ken,只要點過,開放下載excel
          this.$store.dispatch("ExecSevStatusChange", { changeDate: changeDate })
          .then((res) => {
            this.mems = res.data;
    ...
    downloadExcel() {
      ...
      this.searchItem.temp = "sev";
      this.searchItem.reportId = "2-8";
      var fileName = '服務人員失格KK執行結果檔_' + moment(this.changeDate).format("YYYYMMDD") + '.xlsx';
