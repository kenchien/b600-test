          ...
          <v-col width="99%">
            <v-card color="#FEF9E7">
              <v-row dense class="ml-1">
                ...
                <v-col cols="12" sm="2">
                  <v-btn class="ma-4 success" @click="importBankPayFile()">1.DD繳款紀錄txt轉入</v-btn>
                </v-col>
              </v-row>
            </v-card>
          </v-col>
...
export default {
  ...
  methods: {
    ...
    importBankPayFile() {
      this.$refs.obs.validate().then((re) => {
        if (re) {
          ...
          this.$store.dispatch("importBankPayFile", { fileData: this.fileData })
          .then(res => {
    ...
    confirmImportPayRecord() {
      this.$store.dispatch("confirmImportPayRecord",{fileName:this.fileName})
      .then(res => {
        this.execMsg = res.data.message;//顯示訊息在畫面上
        this.canExport = true;
        this.file = null;
    ...
    downloadExcel() {
      ...
      this.searchItem.reportId = "1-12";
      var fileName = 'DD轉入結果檔_' + moment().format("YYYYMMDD") + '.xlsx';
